...
def parse_index_page(text, words):
    ...
    if not headline_result and not title_result and not additional_information_result:
        ...
        phone_number_result = phone_number_searcher.search(text, SearchHelper.LIST_BF_PATTERN)
        result = result | phone_number_result

        telegraph_result = telegraph_searcher.search(text, SearchHelper.LIST_BF_PATTERN)
        result = result | telegraph_result
